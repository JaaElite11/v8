/**
   * Original ALYA-Morou Dika Ardnt.
*/

require('./config')
const { default: ALYAConnect, useSingleFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto } = require("@adiwajshing/baileys")
...
async function startALYA() {
    const ALYA = ALYAConnect({
        logger: pino({ level: 'silent' }),
        printQRInTerminal: true,
        browser: ['ALYA Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    // anticall auto block
    ALYA.ws.on('CB:call', async (json) => {
    const callerId = json.content[0].attrs['call-creator']
    if (json.content[0].tag == 'offer') {
    let pa7rick = await ALYA.sendContact(callerId, global.owner)
    ALYA.sendMessage(callerId, { text: `Sistem otomatis block!\nJangan menelpon bot!\nSilahkan Hubungi Owner Untuk Dibuka !`}, { quoted : pa7rick })
    await sleep(8000)
    await ALYA.updateBlockStatus(callerId, "block")
    }
    })
    ...
    ALYA.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        try {
            let metadata = await ALYA.groupMetadata(anu.id)
            let participants = anu.participants
            for (let num of participants) {
                // Get Profile Picture User
                try {
                    ppuser = await ALYA.profilePictureUrl(num, 'image')
                } catch {
                ...
                // Get Profile Picture Group
                try {
                    ppgroup = await ALYA.profilePictureUrl(anu.id, 'image')
                } catch {
                    ppgroup = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
                }

                if (anu.action == 'add') {
                    ALYA.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*Welcome To ${metadata.subject} Jangan Lupa Baca Deskripsi , Ga Bisa Baca Out Aja Ya Kids @${num.split("@") [ 0 ]}*` })
                } else if (anu.action == 'remove') {
                    ALYA.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*@${num.split("@")[0]} Good Bye Beban Gosah Join Lagi Buang Buang Data Aja ${metadata.subject}*` })
	...
	for (let i of kon) {
	    list.push({
	    	displayName: await ALYA.getName(i + '@s.whatsapp.net'),
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await ALYA.getName(i + '@s.whatsapp.net')}\nFN:${await ALYA.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:okeae2410@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/cak_haho\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
	    })
	}
	ALYA.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted })
    }
    ...
    ALYA.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete Session and Scan Again`); ALYA.logout(); }
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startALYA(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startALYA(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); ALYA.logout(); }
    ...
    /** Send List Messaage
      ...
      *@param {*} quoted
      */
        ALYA.sendListMsg = (jid, text = '', footer = '', title = '' , butText = '', sects = [], quoted) => {
        let sections = sects
        var listMes = {
        text: text,
        footer: footer,
        ...
        ALYA.sendMessage(jid, listMes, { quoted: quoted })
        }
     ...
     * @returns 
     */
        ALYA.send5ButMsg = (jid, text = '' , footer = '', but = []) =>{
        ...
        ALYA.sendMessage(jid, templateMessage)
        }
     ...
     * @returns 
     */
    ALYA.sendVideo = async (jid, path, caption = '', quoted = '', gif = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await ALYA.sendMessage(jid, { video: buffer, caption: caption, gifPlayback: gif, ...options }, { quoted })
    }
     ...
     * @returns 
     */
    ALYA.sendAudio = async (jid, path, quoted = '', ptt = false, options) => {
        let buffer = Buffer.isBuffer(path) ? path : /^data:.*?\/.*?;base64,/i.test(path) ? Buffer.from(path.split`,`[1], 'base64') : /^https?:\/\//.test(path) ? await (await getBuffer(path)) : fs.existsSync(path) ? fs.readFileSync(path) : Buffer.alloc(0)
        return await ALYA.sendMessage(jid, { audio: buffer, ptt: ptt, ...options }, { quoted })
    }
    ...
    ALYA.cMod = (jid, copy, text = '', sender = ALYA.user.id, options = {}) => {
		...
		copy.key.fromMe = sender === ALYA.user.id
